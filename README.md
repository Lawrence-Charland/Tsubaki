...
Tsubaki is a checksum utility for verifying file integrity, comparing directories, and detecting duplicates. It supports multiple hash algorithms (MD5, SHA1, SHA256, SHA512) and offers flexible input sources – directories, file lists, or plain path lists. Multi‑threaded processing and smart buffer sizing make it suitable for large data sets.

## Features

- **Sum mode** – Recursively compute checksums for all files in a directory, or for a file list read from standard input.
- **Merge mode** – Combine two checksum lists (e.g., from two different directories) and prefix each line with `A ` or `B ` for later comparison.
- **Compare mode** – Read a merged list and classify files as modified, moved/copied, added/deleted, or matched.
- **Duplicate mode** – Identify duplicate files based on identical checksums and suggest a deletion command.
- **Multi‑threading** – Automatically enabled for large workloads; thread count and chunking strategy are configurable.
- **Flexible filtering** – Include or exclude subtrees, filter by file size.
...
| Option | Description |
|--------|-------------|
| `--exclude=<dir>` | Exclude files under the given directory (can be repeated). |
...
| `--test`          | Do not compute checksums – only load and filter the file list. |
...
| `--buffer-size=<size>` | Use a static buffer of exactly `<size>` bytes. Default is dynamic (≤64 KB). |
...
If any errors occurred, they are listed at the end. The program exits with status `1` if at least one error was reported.

#### Tips

- Press **Ctrl+C** to stop calculation gracefully. Already processed results are still written to stdout. You can later resume by feeding the half‑finished output back with `cat half.txt | tsubaki sum <type> stdin`.
...
Display a concise help message (the same as shown at the beginning of this README).
...
- **Signal handling** – `SIGINT` (Ctrl+C) is caught. Threads finish their current file and then exit. Already computed results are still written.
- **Memory usage** – The file list is stored entirely in memory. For very large directory trees (millions of files), this may become a bottleneck.
